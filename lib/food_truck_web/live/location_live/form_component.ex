defmodule FoodTruckWeb.LocationLive.FormComponent do
  ...
  def update(%{location: location} = assigns, socket) do
    changeset = Faclities.change_location(location)
  ...
  def handle_event("validate", %{"location" => location_params}, socket) do
    changeset =
      ...
      |> Faclities.change_location(location_params)
  ...
  defp save_location(socket, :edit, location_params) do
    case Faclities.update_location(socket.assigns.location, location_params) do
      {:ok, location} ->
  ...
  defp save_location(socket, :new, location_params) do
    case Faclities.create_location(location_params) do
      {:ok, location} ->
        notify_parent({:saved, location})
